...
async fn lightweight_probe_inner(
    peer_addr: SocketAddr,
    network: Network,
    connection_timeout: Duration,
) -> LightweightProbeResult {
    println!(
        "Starting new lightweight probe connection: peer addr is {:?}",
        peer_addr
    );
    let connection = connect_isolated_tcp_direct(
        network,
    ...
    match connection {
        Err(timeout_error) => {
            println!(
                "Lightweight test connection with {:?} failed due to user-defined timeout of {:?}: {:?}",
                peer_addr, connection_timeout, timeout_error
            );
            ...
            match connection_might_have_failed {
                Err(connection_failure) => {
                    ...
                    match error_classification {
                        ErrorFlavor::Ephemeral(msg) => {
                            println!(
                                "Lightweight test connection with {:?} got an ephemeral error: {:?}",
                                peer_addr, msg
                            );
                        ...
                        ErrorFlavor::Network(msg) => {
                            println!(
                                "Lightweight test connection with {:?} got an network error: {:?}",
                                peer_addr, msg
                            );
                        ...
                        ErrorFlavor::Protocol(msg) => {
                            println!(
                                "Lightweight test connection with {:?} got an protocol error: {:?}",
                                peer_addr, msg
                            );
...
enum LightweightProbeResult {
    ...
    LightweightFail(PeerDerivedData), // increment total_attempts and tcp_connections_ok and protocol_negotiations_ok

    // Nominal reply to the headers request
    LightweightOK(PeerDerivedData), // increment total_attempts and tcp_connections_ok and protocol_negotations_ok and valid_headers_reply_ok
}
